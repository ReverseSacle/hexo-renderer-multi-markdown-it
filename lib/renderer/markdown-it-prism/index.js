...
module.exports = function(md, options) {
    ...
    md.renderer.rules.fence = (tokens, idx, options, env, self) => {
        ...
        if (prismLang && (code = Prism.highlight(unescapeSwigTag(text), prismLang, langToUse))) {
            ...
            for (let i = 0, len = lines.length; i < len; i++) {
                ...
                if( !_add  && -1 != (index = line.indexOf('<span class="token comment">'))){ 
                    comment_pre = line.slice(0,index + 28);
